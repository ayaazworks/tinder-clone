...
const OnboardingPage = () => {
  ...
  const { currentStep, profile, updateProfile, nextStep, prevStep, gotoStep, getOverallCompletion, getStepCompletion, resetObboarding } = useOnboading(user?.uid);
  ...
  const handlelogout = async () => {
    try {
      await logout();
      resetObboarding(); // Clear local storage on logout
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      resetObboarding(); // Use the hook function to clear storage
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 1: return <BasicInfoStep profile={profile} updateProfile={updateProfile} />;
      case 2: return <InterestedStep profile={profile} updateProfile={updateProfile} />;
      case 3: return <PhysicalStep profile={profile} updateProfile={updateProfile} />;
      case 4: return <LifeStyleStep profile={profile} updateProfile={updateProfile} />;
      case 5: return <LocationStep profile={profile} updateProfile={updateProfile} />;
      case 6: return <JobEducationBio profile={profile} updateProfile={updateProfile} />;
      case 7: return <PhotoStep profile={profile} updateProfile={updateProfile} />;
      default: return null;
    }
  };
  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-indigo-50'>
      ...
      <div className='md:hidden bg-white border-b shadow-sm'>
        <div className='p-4'>
          <div className='flex items-center justify-between mb-3'>
            ...
            <span className='text-sm font-medium text-gray-900'>{getOverallCompletion()}%</span>
          </div>
      ...
      <div className='flex justify-center items-start'>
        ...
        {/* Desktop Sidebar - Fixed Gradients and Typos */}
            ...
            {steps.map((step) => (
              ...
              >
                <div className={`flex w-10 h-10 rounded-full items-center justify-center text-lg ${getStepCompletion(step.id) ? 'bg-green-500 text-white' : currentStep === step.id ? 'bg-white text-pink-500' : 'bg-white/20'}`}>
                  {getStepCompletion(step.id) ? (<Check className='h-4 w-4' />) : (step.icon)}
                </div>
                <div>
